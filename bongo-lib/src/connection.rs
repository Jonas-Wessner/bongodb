...
impl Connection {
    ...
    pub fn query<T: FromRow>(&mut self, select_sql: &str) -> QueryResult<T> {
    ...
    pub fn insert_multiple<T>(&mut self, inserts: &[T]) -> ExecuteResult
    where
        T: InsertQuery,
    {
        let mut query = T::insert_query_head();

        query.push(' ');

        for insert in inserts.iter() {
            query.push_str(format!("({}),", insert.insert_query_values()).as_str());
        }

        dbg!(&query[..query.len() - 1]);

        self.execute(format!("{};", &query[..query.len() - 1]).as_str())
    }
}
